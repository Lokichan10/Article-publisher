...
if ($result->num_rows > 0) {
    ...
    // Вывод данных
    echo '<div class="card" style="width: 70%; margin-bottom: 1rem; margin-left: 220px; margin-top: 50px;">
            <div class="card-body">
                <h1 class="card-title">' . htmlspecialchars($title) . '</h1>
                <h6 class="card-subtitle mb-2 text-body-secondary">' . htmlspecialchars($content) . '</h6>
                <p class="card-text">' . htmlspecialchars($publish_date) . '</p>
            </div>
          </div>';
} else {
...
// Вывод комментариев
$sql = "SELECT * FROM comments WHERE post_id = $id ORDER BY publish_date DESC";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo '<div class="card" style="width: 70%; margin-bottom: 1rem; margin-left: 220px; margin-top: 10px;">
                <div class="card-body">
                    <h5 class="card-title">' . htmlspecialchars($row["author"]) . '</h5>
                    <p class="card-text">' . htmlspecialchars($row["content"]) . '</p>
                    <p class="card-text"><small class="text-muted">' . htmlspecialchars($row["publish_date"]) . '</small></p>
                </div>
              </div>';
    }
} else {
...
<div class="card" style="width: 70%; margin-bottom: 1rem; margin-left: 220px; margin-top: 10px;">
    <div class="card-body">
